...
orders_without_date_frontend_router = APIRouter(
    prefix='/without-date'
)


@orders_without_date_frontend_router.get("/", response_class=HTMLResponse)
async def view_orders_without_date(
    request: Request,
    ...
    context = {
        ...
        "title_name": "Заявки без даты",
        **employee_data.__dict__,
    }
    return templates.calendar.TemplateResponse(
        "orders_without_date/orders_without_date_home.html", context
    )
