...
orders_search_frontend_router = APIRouter(
    prefix='/search'
)


@orders_search_frontend_router.get("/", response_class=HTMLResponse,)
async def view_orders_search(
    request: Request,
    ...
    context = {
        ...
        "title_name": "Поиск заявок",
        **employee_data.__dict__
    }

    return templates.calendar.TemplateResponse(
        "orders_search/orders_search_home.html", context)
